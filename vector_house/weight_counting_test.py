...
@run_with_db
def test_weight_counting(db: WikiDatabase) -> None:
    title1 = "Clouds"
    ...
    for page_title, text in texts.items():
        doc_id = db.insert_document(page_title, text)
        freq_dict = ind.lemmatize_text(text)
        ind.update_abs_freq(freq_dict, terms, doc_id, absolute_freq, db)

    relative_freq = ind.count_relative_freq(absolute_freq, terms)
    ind.weights_to_db(db, relative_freq, terms, len(texts))

    print("RELATIVE FREQUENCIES: ")
    for term_id, dict in relative_freq.items():
        print(db.get_term_by_id(term_id), "( id:", term_id, ") -> ", dict)

    doc_id1 = db.get_doc_id(title1)
    doc_id2 = db.get_doc_id(title2)
    doc_id3 = db.get_doc_id(title3)

    cloud_id = db.get_term_id("cloud")  # cloud is in all texts
    assert db.get_value_by_ids(cloud_id, doc_id1) == 0
    assert db.get_value_by_ids(cloud_id, doc_id2) == 0
    assert db.get_value_by_ids(cloud_id, doc_id3) == 0

    substantial_id = db.get_term_id("substantial")  # substantial only in text1
    assert db.get_value_by_ids(substantial_id, doc_id1) == 1.58
    assert db.get_value_by_ids(substantial_id, doc_id2) == 0
    assert db.get_value_by_ids(substantial_id, doc_id3) == 0

    assert db.get_value_by_ids(200, doc_id1) == 0  # term id does not exist
    assert db.get_value_by_ids(200, doc_id2) == 0
    assert db.get_value_by_ids(200, doc_id3) == 0

    assert db.get_value_by_ids(cloud_id, 50) == 0  # document id does not exist

    earth_id = db.get_term_id("earth")  # earth 2x in text1, 1x in text2, 0x in text3
    assert db.get_value_by_ids(earth_id, doc_id1) == 0.58
    assert db.get_value_by_ids(earth_id, doc_id2) == 0.29
    assert db.get_value_by_ids(earth_id, doc_id3) == 0
